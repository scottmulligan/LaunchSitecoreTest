        ...
        chartElement.interactiveLayer.tooltip.contentGenerator(function (d) {
          ...
          for (seriesIndex = 0; seriesIndex < dataLength; seriesIndex++) {
            ...
            if (chartType === models.constants.chartType.area) {
              color = (data[index].legendState === models.constants.legendState.disengaged) ? models.colors.getPaletteColor(palette, index) : d3.rgb(models.colors.getPaletteColor(palette, index)).darker(1);
            } else {
      ...
      */
      removeOverlappingLabels: function (canvas, hasBarInCombinationChart, dataLength, chartType) {
        ...
        if (chartType === models.constants.chartType.combination) {
          ...
          drawingCanvasRight = canvas.select(".nv-x.nv-axis.nvd3-svg").node().getBoundingClientRect().right +5;
        } else {
          drawingCanvasRight = canvas.select(".nv-y.nv-axis.nvd3-svg .nvd3.nv-wrap.nv-axis g.tick:first-of-type line").node().getBoundingClientRect().right;
        }
        ...
        for (var j = startIndex; j < endIndex; j++) {
          var c = ticks[0][j],
            n = ticks[0][j + 1];
          if (c && c.getBoundingClientRect && c.getBoundingClientRect().right > (drawingCanvasRight - 5)) {
            d3.select(c).remove();
            continue;
          }
